...
fn get_place_update(map: &Vec<Vec<Place>>, y: usize, x: usize, place: &Place) -> Option<Place> {
    match place {
        ...
        Place::Occupied => match get_occupied_neighbour_count(map, y, x) {
            neighbours if neighbours >= 5 => Some(Place::Empty),
            _ => None,
        },
...
fn get_occupied_neighbour_count(map: &Vec<Vec<Place>>, y: usize, x: usize) -> usize {
    DIRECTIONS_8
        ...
        .filter(|&(x_diff, y_diff)| {
            let mut new_x = x as i32 + x_diff;
            let mut new_y = y as i32 + y_diff;

            let width = map.first().unwrap().len();
            let height = map.len();

            while in_bounds(new_x, new_y, width, height) && map[new_y as usize][new_x as usize] == Place::Floor {
                new_x += x_diff;
                new_y += y_diff;
            }

            return in_bounds(new_x, new_y, width, height) && map[new_y as usize][new_x as usize] == Place::Occupied;
        })
        .count()
}
